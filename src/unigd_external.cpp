...
UNIGD_RENDER_HANDLE api_render_create(UNIGD_HANDLE ugd_handle,
...
{
  // Placeholder in case a renderer lookup ever needs to alloc (e.g. for dynamic renderer
  // adding/removing)
}
...
// clang-format on
{
  R_RegisterCCallable("unigd", "api_v1_create",
                      reinterpret_cast<DL_FUNC>(unigd::ex::api_v1_create));
  R_RegisterCCallable("unigd", "api_v1_destroy",
                      reinterpret_cast<DL_FUNC>(unigd::ex::api_v1_destroy));
}
