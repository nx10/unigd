...
{
  return CreateWindowEx(0, UNIGD_WINDOW_CLASS_NAME, TEXT("unigd"), 0, 0, 0, 0, 0,
...
void ipc_open()
{
  ...
  if (!window_class_exists())
  {
    ...
    // we have to re-use the (unique) window class, but should not reuse the message
    // window. Otherwise we could not destroy the window while it is in use. As the
    // callback lives inside the window class, messages be received only once.
